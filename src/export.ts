...
const ROLE: Record<string, string> = { user: "用户", toolResult: "工具", assistant: "AI" };

/** Every image in the file, all branches, in the order they were written. */
export function sessionImages(raw: string): SessionImage[] {
  ...
  for (const line of raw.split("\n")) {
    ...
    for (const p of e.message.content) {
      if (p?.type === "image" && typeof p.data === "string") out.push({ data: Buffer.from(p.data, "base64"), mimeType: String(p.mimeType ?? ""), role: ROLE[e.message.role] ?? "其他" });
    }
  }
  return out;
}
 ...
 */
export async function exportImages(sessions: { record: SessionRecord; title: string }[], dir: string): Promise<ExportResult> {
  await mkdir(dir, { recursive: true });
  const existing = await readdir(dir).catch(() => [] as string[]);
  const result: ExportResult = { dir, sessions: 0, images: 0, written: 0 };
  for (const { record, title } of sessions) {
    const images = sessionImages(await readFile(record.path, "utf8"));
    ...
    for (const [i, img] of images.entries()) {
      const file = join(folder, `${String(i + 1).padStart(3, "0")}-${img.role}.${EXT[img.mimeType] ?? "bin"}`);
      result.images++;
      if ((await stat(file).catch(() => undefined))?.size === img.data.length) continue;
